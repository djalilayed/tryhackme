...
# work with python 3.9 on tryhackme attackbox where you need to run: pip install "web3==5.31.1"
...
# Check if connected
assert w3.isConnected(), "Failed to connect to RPC!"  # Changed from is_connected() to isConnected()
...
transaction = contract.functions.transfer(decoded_data, 1000).build_transaction({
    'chainId': 31337,
    'gas': 2000000,
    'gasPrice': w3.eth.gas_price,  # Changed from w3.to_wei()
    'nonce': nonce
})
